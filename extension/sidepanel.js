...
function _aiMemoryKey(hostname) {
  ...
  if (shared?.memoryKey) return shared.memoryKey(hostname);
...
function _aiClassifyPageContext(pageContext) {
  const shared = _aiShared();
  if (shared?.classifyPageContext) return shared.classifyPageContext(pageContext);
  return {
    type: "HTML page with no PDF found",
    stepsSummary: "Use Help me identify this page first.",
    warnings: [],
  };
}
...
async function askGemini(userMessage, pageContext) {
  const shared = _aiShared();
  if (!shared?.askGemini) throw new Error("AI helper unavailable.");
  return shared.askGemini({
    userMessage,
    pageContext,
    messages: _aiMessages,
    currentMemory: _aiCurrentMemory,
    storageGet: _spStorageGet,
  });
}

async function _aiSaveMemory(hostname, pageContext) {
  const shared = _aiShared();
  if (!shared?.saveHostMemory) return;
  const payload = await shared.saveHostMemory({
    hostname,
    pageContext,
    storageSet: _spStorageSet,
  });
  if (!payload) return;
